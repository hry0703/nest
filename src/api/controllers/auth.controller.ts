...
export class AuthController {
  ...
  async denyQrCode(@Query('token') token: string, @Res() res: Response) {
    await this.redisService.set(
      `qrcode_login:${token}`,
      QrCodeStatusEnum.DENIED,
      300,
    );
    return res.json({ success: true, message: '已拒绝授权' });
  }
  @Post('qrcode-authorize')
  async authorizeQrCode(
    @Query('token') token: string,
    @Res() res: Response,
    @Body('appAccessToken') appAccessToken: string,
  ) {
    //先获取到老的二维码的状态
    ...
    if (currentStatus !== QrCodeStatusEnum.SCANNED) {
      return res
        .status(400)
        .json({ success: false, message: '二维码未扫描或者已过期' });
    }
    if (!appAccessToken) {
      //如果没有提供用户登录的Token，服务器无法知道是哪个用户要授权登录
      return res
        .status(400)
        .json({ success: false, message: '用户信息未提供' });
    }
    try {
      const decodedUser = this.jwtService.verify(appAccessToken, {
        secret: this.configurationService.jwtSecret,
      });
      await this.redisService.set(
        `qrcode_login:${token}`,
        QrCodeStatusEnum.AUTHORIZED,
        300,
      );
      //保存此二维授权登录的用户ID
      await this.redisService.set(
        `qrcode_login_user:${token}`,
        decodedUser.id,
        300,
      );
  ...
  async checkQrcodeStatus(@Query('token') token: string, @Res() res: Response) {
    ...
    if (status === QrCodeStatusEnum.AUTHORIZED) {
      ...
      if (!user) {
        return res
          .status(404)
          .json({ status: QrCodeStatusEnum.ERROR, message: '用户不存在' });
      }
      const tokens = this.createJwtTokens(user);
