...
export class AuthMiddleware implements NestMiddleware {
  ...
  async use(req: any, res: any, next: (error?: Error | any) => void) {
    ...
    if (this.hasPermission(user, req.originalUrl)) {
      ...
      res
        ...
        .render('error', { message: '无权限访问此页面', layout: false });
    }
  }
  ...
  private getUserMenuTree(accessTree: Access[], userAccessIds: number[]) {
    return accessTree.filter((access) => {
      ...
      if (
        access.type === AccessType.PAGE ||
        access.type === AccessType.BUTTON ||
        !userAccessIds.includes(access.id)
      ) {
        return false;
      }
      if (access.children) {
        access.children = this.getUserMenuTree(
          access.children as any,
          userAccessIds,
        ) as any;
      }
      return true;
