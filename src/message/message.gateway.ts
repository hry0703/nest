...
export class MessageGateway {
  ...
  handleUserJoined(
    ...
    //通过ws服务器向所有连接的客户端进行广播 包括当前socket自己
  ...
  handleCreateMessage(
    ...
    if (recipient) {
      ...
      let recipientSocket = Array.from(
        this.server.sockets.sockets.values(),
      ).find((socket) => socket.data.username == recipient);
      if (recipientSocket) {
        recipientSocket.emit('messageCreated', { username, message }); // 给@的人发消息
        socket.emit('messageCreated', { username, message }); // 给自己也发 让自己能看见这条信息
      }
    } else if (room) {
